// Package migrate applies the embedded billing-schema migrations at
...
// replicas starting at once). Arbitrary but stable, distinct from the
// identity runner's key.
const advisoryLockKey = 0x2dab_e7_b1
...
func Run(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if _, err := pool.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS billing`); err != nil {
...
func apply(ctx context.Context, pool *pgxpool.Pool, m Migration) error {
	...
	var applied bool
	err = tx.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM billing.schema_migrations WHERE version = $1)`,
	...
	if _, err := tx.Exec(ctx,
		`INSERT INTO billing.schema_migrations (version, name) VALUES ($1, $2)`,
