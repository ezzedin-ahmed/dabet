// Package migrate applies the embedded review-schema migrations at
// startup using a tiny version-table runner (same pattern as
// user-service/internal/migrate). Each migration runs once, inside a
// transaction, serialised across replicas by an advisory lock.
package migrate
...
// replicas starting at once). Arbitrary but stable, and distinct from the
// other services' keys.
const advisoryLockKey = 0x2eb1_e4_0c
...
func Run(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if _, err := pool.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS review`); err != nil {
		return fmt.Errorf("ensure review schema: %w", err)
	}
	if _, err := pool.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS review.schema_migrations (
...
func apply(ctx context.Context, pool *pgxpool.Pool, m Migration) error {
	...
	var applied bool
	err = tx.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM review.schema_migrations WHERE version = $1)`,
	...
	if _, err := tx.Exec(ctx,
		`INSERT INTO review.schema_migrations (version, name) VALUES ($1, $2)`,
